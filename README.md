...
- **Voice-activated vision.** Your flashlight runs on your voice. Speak to refill the beacon meter (0-100), which drives the flashlight radius (0-280px). Go silent and the light drains at 2-6 per second. Whisper to maintain a dim glow, talk for full brightness, shout to flood the room. But every sound also feeds the monster's suspicion meter.
...
|-----|----------|-----------------|
| [Text to Speech](https://elevenlabs.io/docs/api-reference/text-to-speech) | Radio bait (real-time), lore tapes (6), tutorials (4), whisperer voices (9), map narration (1), radio hints (4) | 24 static + on-demand |
| [Sound Effects](https://elevenlabs.io/docs/api-reference/sound-effects) | Monster vocals (8), footsteps, doors, breaker, death, radio throw | 24 |
...
The offline pipeline (`scripts/generate-audio.ts`) uses the [`@elevenlabs/elevenlabs-js`](https://www.npmjs.com/package/@elevenlabs/elevenlabs-js) SDK (v2.44.0) to batch-generate all 53 catalog entries. 47 MP3 files are currently on disk.
...
graph TD
    ...
    subgraph "Game Logic"
        ...
        GAME --> JUMPER[jumper.ts]
        GAME --> WHISPERER[whisperer.ts]
        GAME --> ROOMS[rooms.ts]
        GAME --> INPUT[input.ts]
    ...
    subgraph "Audio Pipeline"
        ...
        AUDIO --> CATALOG[audio-catalog.ts<br>53 entries]
...
src/
  ...
  audio-catalog.ts     53 audio asset definitions with generation prompts
...
assets/
  atlas.json           Sprite manifest (25 entities, 241 frames)
  audio/               47 MP3 files
...
34 TypeScript source files. 120 public exports.
...
flowchart LR
    ...
    BAND -->|silent| DRAIN[Drain 2-6/sec]
    BAND -->|whisper| R1[Refill 1/sec]
    BAND -->|normal| R2[Refill 3/sec]
    BAND -->|shout| R3[Refill 12/sec]
...
|------------|---------------|---------------|---------------------|
| Silent | < 0.0075 | Drain 2/sec (6/sec below 30) | 0.0x |
| Whisper | 0.0075 - 0.015 | Refill 1/sec | 0.5x |
| Normal | 0.015 - 0.045 | Refill 3/sec | 1.0x |
| Shout | > 0.045 | Refill 12/sec | 2.0x |
...
53 audio assets defined in `src/audio-catalog.ts`. 47 MP3 files on disk.

| Category | In Catalog | On Disk | Source API | Examples |
|----------|-----------|---------|------------|---------|
| Ambient | 5 | 5 | Music | reception, cubicles, server, stairwell, archives |
| Monster Vocals | 8 | 6 | Sound Effects | patrol_breath, alert_growl, hunt_screech, charge_roar, attack_lunge, idle_howl |
| SFX | 16 | 12 | Sound Effects | footsteps, doors, breaker, keycard, death_thud, static_burst, radio_throw |
| Radio Hints | 4 | 4 | TTS (eleven_v3) | radio_intro, keycard_hint, breaker_hint, exit_hint |
| Whisperer | 9 | 9 | TTS (Bella voice) | whisper_01..08, whisper_fade |
| Lore Tapes | 7 | 7 | TTS (Adam voice) | tape_01..06, tape_map_fragment |
| Tutorials | 4 | 4 | TTS (Adam voice) | tutorial_t0..t3 |

6 catalog entries are not on disk: `confused_growl`, `monster_growl_close`, `locker_close`, `locker_open`, `locker_door_creak`, `desk_crouch`. The game runs without them (silent skip, no crash).
...
flowchart TD
    ...
    SAVE --> ATLAS[Build atlas.json<br>25 entities, 241 frames]
...
|----------|----------|---------|-------------|
| `ELEVENLABS_API_KEY` | Yes | (none) | [ElevenLabs](https://elevenlabs.io) API key. Used by `tts.ts` at runtime (injected via Vite `define` as `__ELEVENLABS_API_KEY__`) and by `generate-audio.ts` for offline asset generation. |

The API key is injected into the client bundle by Vite at build time. It is visible in the browser. For production, proxy the TTS call through a backend.
...
- The [ElevenLabs](https://elevenlabs.io) API key is exposed in the client bundle. Intentional for a hackathon demo. A production build needs a server-side proxy.
- Microphone sensitivity varies by hardware. The suspicion curve and beacon drain rates were calibrated on one microphone. Other setups may trigger too easily or not at all.
- 6 audio assets are defined in the catalog but not generated on disk. The game runs without them silently.
