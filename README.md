...
- **3 monster types.** The Listener patrols with a 6-state AI driven by microphone input. The Jumper ambushes from ceiling vents (drops from above) and floor vents (emerges from grates at ground level, crawls toward player). Both variants share peek and fake-out behaviors for psychological tension. The Whisperer spawns at room corners and drains your beacon capacity with eerie voice lines generated by [ElevenLabs TTS](https://elevenlabs.io/docs/api-reference/text-to-speech).
...
- **Hiding spots.** Lockers (100% hidden, instant suspicion drop) and desks (80% hidden, 4x decay multiplier, animated enter/idle/exit sequence). The Listener cannot detect a player inside a locker.
- **Guidance arrow.** A floating arrow above the player's head points toward the next objective (keycard, breaker, map fragment). Disappears once the map fragment is acquired and the minimap takes over.
- **Minimap.** Hidden until the player picks up the Map Fragment in Archives. Triggers a TTS narration line. Renders a 5-room hub-and-spokes layout with visited room tracking, a pulsing player dot, and pulsing objective room highlights. Persists through death.
...
The offline pipeline (`scripts/generate-audio.ts`) uses the [`@elevenlabs/elevenlabs-js`](https://www.npmjs.com/package/@elevenlabs/elevenlabs-js) SDK (v2.44.0) to batch-generate all 60 catalog entries. All 60 MP3 files are on disk.
...
graph TD
    ...
    subgraph "Audio Pipeline"
        ...
        AUDIO --> CATALOG[audio-catalog.ts<br>60 entries]
...
src/
  ...
  jumper.ts            Ceiling and floor ambush predator (10-state FSM)
  ...
  audio-catalog.ts     60 audio asset definitions with generation prompts
  ...
  hud.ts               Suspicion meter, sprite/text prompts, status indicators
  ...
  guidance-arrow.ts    Floating arrow guiding player to next objective
scripts/
  slice.py             Atlas pipeline orchestrator
  atlas_config.py      72 atlas profile definitions
...
assets/
  atlas.json           Sprite manifest (25 entities, 257 frames)
  audio/               60 MP3 files (all catalog entries present)
  player/              49 player sprite PNGs
  monster/             27 monster sprite PNGs
  props/               12 prop sprite PNGs
  *.png                Room backgrounds, title, gameover
```

35 TypeScript source files. 122 public exports.
...
| Cubicles | 3344x941 | Listener, 1 Ceiling Jumper, 2 Floor Jumpers | Keycard, glass shards, lore tape 02. 7 foreground dividers, 2 desks, 1 locker, 1 radio. Vent to Stairwell |
| Server | 3344x941 | Listener, 2 Ceiling Jumpers, 1 Floor Jumper | Breaker, wire, lore tape 03. Upper floor with ladder at x=1400. 2 lockers, 1 radio |
| Stairwell | 3344x941 | Listener, 2 Ceiling Jumpers, 1 Floor Jumper, Whisperer (40%) | Lore tape 06. 1 desk. Vent to Cubicles. EXIT requires keycard |
...
| CHARGE | 4.5 (6.75 on dash) | Rushes player. 25% chance of a 600ms dash burst at 1.5x speed |
...
Ceiling and floor ambush predator. Two variants share the same 10-state FSM. Ceiling variant hides dormant in vent hotspots above the player's path, drops down on real attack. Floor variant emerges from grates at ground level, stands up, and crawls toward the player. Both are triggered when the player walks within range with a probability roll: 25% real attack, 40% fake-out (emerge then retreat with a hiss), 35% peek (slow idle frames through the vent grate). Floor variant idle is visible when the player is within 250px. Cooldowns prevent spam (3s after peek/fake, 5s after real attack). States: dormant, peeking, fake_attacking, triggered, falling, emerging, getting_up, crawling, attacking, retreating. Floor-exclusive states: emerging, getting_up, crawling. Ceiling-exclusive: triggered, falling. Floor variant uses per-variant frame remapping (getup, walk frames) to avoid double-vent visual artifacts from the ceiling emerge/retreat frames. 5 ceiling + 4 floor across 3 rooms.
...
60 audio assets defined in `src/audio-catalog.ts`. All 60 MP3 files are on disk.
...
| SFX | 20 | Sound Effects | footsteps, doors, breaker, keycard, death_thud, hiding (locker/desk), static_burst, radio_throw, monster_dash_screech, jumper SFX |
...
72 atlas profiles defined in `atlas_config.py`. Pipeline commands:
...
- All 60 audio catalog entries have corresponding MP3 files on disk.
